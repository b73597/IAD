...
-(void)touchesBegan:(NSSet *)touches withEvent:(UIEvent *)event {
    if(self.nodeCreditBackground.hidden){
        ...
        if([node.name isEqualToString:@"high_score_btn"]){
            [[NSNotificationCenter defaultCenter] postNotificationName:@"SHOW_HIGH_SCORE" object:nil];
        }else if([node.name isEqualToString:@"play_button"]){
